...
**If Egeria runs on the same machine as this Advisor, point `EGERIA_VIEW_SERVER_URL` at `localhost`, never at that machine's own public hostname** — even if the deployment *is* a public-hostname demo system. A machine generally cannot reach its own forwarded port via its public IP/hostname (NAT hairpinning): the outbound TCP connection just hangs until it times out, while the exact same request succeeds instantly over `localhost`. This is easy to misdiagnose as a slow/overloaded Egeria server or an insufficient timeout — it looks like a timeout, raising timeouts changes the symptom (how long it hangs before failing) without fixing anything, and the fact that some *other* client (e.g. a browser reaching in externally, or another local tool hitting Egeria directly) responds instantly is the tell that it's connectivity, not load. Confirm with `curl -k --max-time 15 https://<host>:9443/...` (hangs) vs `curl -k https://localhost:9443/...` (should be milliseconds) before spending time on timeout tuning.
